 ...
 *
 * Credit to Quorafind
 * @see https://github.com/Quorafind/Obsidian-Card-Library
 *
 */
...
function processSelector(selector, lightOrDark) {
  ...
  if (lightOrDark) {
    ...
    for (let i = 0; i < prefixList.length; i++) {
      newSelector +=
        lightOrDark + " " + prefixList[i] + " " + tempSelector + ", ";
    }
    for (let i = 0; i < dialogPrefix.length; i++) {
      newSelector +=
        ...
        " > " +
        dialogPrefix[i] +
        (tempSelector.trim().startsWith(".")
          ? tempSelector
          : " " + tempSelector) +
        (i === dialogPrefix.length - 1 ? "" : ", ");
    }
  } else {
    for (let i = 0; i < prefixList.length; i++) {
    ...
    for (let i = 0; i < dialogPrefix.length; i++) {
      newSelector +=
        "body > " +
        dialogPrefix[i] +
        (selector.trim().startsWith(".") ? selector : " " + selector) +
        (i === dialogPrefix.length - 1 ? "" : ", ");
    }
  }
  return newSelector;
...
const prefix = prefixSelector({
  prefix:
    ":is(.twcss, div[data-radix-popper-content-wrapper], div[role='dialog'], span[data-radix-focus-guard])",
  transform: (_, selector, __, _filePath, _rule) => {
    ...
    // else if (
    // 	selector.includes(".view-content") ||
    // 	selector.includes(".card-library-settings")
    // ) {
    // 	return selector;
    // }
    else {
